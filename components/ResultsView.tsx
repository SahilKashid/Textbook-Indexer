...
export const ResultsView: React.FC<ResultsViewProps> = ({ data, originalFile, usePrintedPageNumbers }) => {
  ...
  const generateCompositePdf = async () => {
    ...
    try {
      ...
      {
         let pages = 1;
         let cursorY = pageHeight - margin - 60;
      ...
      sortedKeys.forEach(letter => {
         ...
         groupedIndex[letter].forEach(item => {
             ...
             let currentX = drawX + lastLineW + 8; // Start after term
             ...
             pageNums.forEach((pNum, idx) => {
                 ...
                 const match = originalPageStr.match(/(\d+)/);
                 const pInt = match ? parseInt(match[0]) : NaN;
                 if (!isNaN(pInt)) {
                 ...
                 // Wrap page numbers if they exceed column width
  ...
  const downloadData = (format: 'json' | 'md' | 'csv' | 'pdf') => {
    if (format === 'pdf') {
      ...
      // ... (Rest of existing simple print fallback remains unchanged for brevity, as it is robust)
      printWindow.document.write('<html><body><p>Please use a PDF file for full features.</p></body></html>');
    ...
    if (format === 'json') {
      content = JSON.stringify(data, null, 2); mimeType = 'application/json'; filename = 'analysis.json';
    } else if (format === 'md') {
      content = `# ToC\n${data.toc.map(t => `- ${t.title} (p${t.pageNumber})`).join('\n')}`; mimeType = 'text/markdown'; filename = 'analysis.md';
    } else if (format === 'csv') {
      content = 'Term,Pages\n' + data.index.map(i => `"${i.term}","${i.pageNumbers.join(';')}"`).join('\n'); mimeType = 'text/csv'; filename = 'index.csv';
    }
    ...
    <div className="w-full h-full bg-[#0A0A0A] border border-white/5 rounded-2xl overflow-hidden flex flex-col shadow-2xl shadow-black/80 ring-1 ring-white/[0.02]">
      <div className="relative z-20 bg-black/40 border-b border-white/5 px-4 h-16 flex items-center justify-between gap-4 backdrop-blur-md">
        <div className="flex bg-white/[0.03] p-1 rounded-lg border border-white/[0.02]">
          <button onClick={() => setActiveTab('toc')} className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'toc' ? 'bg-zinc-800 text-white shadow-sm ring-1 ring-white/5' : 'text-gray-500 hover:text-gray-300'}`}>
            <Book size={13} /> Table of Contents
          </button>
          <button onClick={() => setActiveTab('index')} className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'index' ? 'bg-zinc-800 text-white shadow-sm ring-1 ring-white/5' : 'text-gray-500 hover:text-gray-300'}`}>
            <List size={13} /> Index
          </button>
        </div>
        <div className="flex items-center gap-3">
          {activeTab === 'index' && (
             <div className="relative group hidden sm:block">
               ...
               <input type="text" placeholder="Filter..." className="w-48 pl-8 pr-3 py-1.5 bg-black/20 border border-white/10 rounded-full text-xs text-gray-200 focus:outline-none focus:border-white/20 focus:bg-white/5 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
             </div>
          )}
           <div className="relative group z-30">
              <button disabled={isGeneratingPdf} className={`flex items-center gap-2 px-4 py-1.5 bg-white text-black hover:bg-gray-200 rounded-lg text-xs font-medium transition-colors ${isGeneratingPdf ? 'opacity-70 cursor-wait' : ''}`}>
              ...
              {!isGeneratingPdf && (
                <div className="absolute right-0 top-full mt-2 w-40 bg-[#111] rounded-xl shadow-2xl border border-white/10 hidden group-hover:block overflow-hidden backdrop-blur-xl">
                  <div className="p-1">
                    <button onClick={() => downloadData('pdf')} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/10 hover:text-white transition-colors rounded-lg flex items-center justify-between">
                    ...
                    <button onClick={() => downloadData('csv')} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/10 hover:text-white transition-colors rounded-lg">CSV</button>
                  </div>
                </div>
              )}
           </div>
      ...
      <div className="flex-1 overflow-hidden relative bg-black/20">
        {activeTab === 'toc' ? (
          <div className="h-full overflow-y-auto p-8 custom-scrollbar">
            {data.toc.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-2"><Book size={32} className="opacity-20" /><p className="text-sm font-light">No chapters detected.</p></div>
            ) : (
              <div className="max-w-3xl mx-auto space-y-1">
                {data.toc.map((item, idx) => (
                  <div key={idx} className={`relative flex items-baseline py-2 px-4 rounded-lg group hover:bg-white/[0.02] transition-colors ${item.level === 1 ? 'mt-6 mb-2' : ''}`}>
                    {item.level > 1 && <div className="absolute left-4 top-0 bottom-1/2 w-px bg-white/5 border-l border-dashed border-white/10"></div>}
                    {item.level > 1 && <div className="absolute left-4 top-1/2 w-3 h-px bg-white/10"></div>}
                    <div className={`flex-1 flex items-baseline gap-4 ${item.level === 1 ? 'ml-0' : item.level === 2 ? 'ml-8' : 'ml-16'}`}>
                      <span className={`tracking-wide ${item.level === 1 ? 'text-sm font-medium text-white' : 'text-xs text-gray-400 group-hover:text-gray-200'}`}>{item.title}</span>
                      <div className="flex-1 border-b border-dashed border-white/10 opacity-30 mx-2 relative top-[-4px]"></div>
                      <span className={`font-mono text-[10px] ${item.level === 1 ? 'text-white' : 'text-gray-600 group-hover:text-gray-400'}`}>{item.pageNumber}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="h-full flex flex-col">
             <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
                {filteredIndex.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-2"><Search size={32} className="opacity-20" /><p className="text-sm font-light">No matching terms found.</p></div>
                ) : (
                      ...
                      <div key={idx} className="flex flex-col p-4 rounded-xl border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-colors group">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex flex-wrap justify-end gap-1 ml-2 max-w-[50%]">
                             {item.pageNumbers.slice(0, 3).map((p, pIdx) => (<span key={pIdx} className="inline-block bg-white/5 text-gray-500 text-[10px] px-1.5 py-0.5 rounded font-mono border border-white/5">{p}</span>))}
                             {item.pageNumbers.length > 3 && <span className="text-[10px] text-gray-600 px-1">+{item.pageNumbers.length - 3}</span>}
                          </div>
                        </div>
                      </div>
                    ))}
